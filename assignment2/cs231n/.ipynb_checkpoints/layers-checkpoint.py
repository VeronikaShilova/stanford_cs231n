...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = x_norm, x_centered, 1. / (var + eps)**0.5, gamma
...
def layernorm_backward(dout, cache):
    ...
    dmean = -np.sum(dx_normed, axis=1, keepdims=True) * inv_std
    dvar = -0.5 * np.sum(dx_normed * x_norm, axis=1, keepdims=True) * inv_std**2
    dx = dx_normed * inv_std + dmean / d + 2 * x_centered * dvar / d 
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(out_height):
        for j in range(out_width):
            ...
            out_slice = x_slice_flattened.dot(w.reshape((F, -1)).T) + b
            out[:, :, i, j] = out_slice
    ...
    for f in range(F):
        ...
        for i in range(0, last_row, stride):
            ...
            for j in range(0, last_col, stride):
                dx_pad[:, :, i:(i+HH), j:(j+WW)] += w[f] * dout[:, f, i_out, j_out][:,None,None,None]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            ...
            for i in range(out_height):
                for j in range(out_width):
                    out[n, c, i, j] = np.amax(x_curr[i*stride:i*stride + pool_height, 
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(out_height):
                for j in range(out_width):
                    x_curr = x[n, c, i*stride:i*stride + pool_height, j*stride:j*stride + pool_width]
                    ind_h, ind_w = np.unravel_index(x_curr.argmax(), x_curr.shape)
